    ...
    def render(self, camera, means2D, cov2d, color, opacity, depths):
        ...
        for h in range(0, camera.image_height, TILE_SIZE):
            for w in range(0, camera.image_width, TILE_SIZE):
                ...
                tile_min = torch.tensor([w, h], device="cuda")
                tile_max = torch.tensor([w + TILE_SIZE, h + TILE_SIZE], device="cuda")

                in_mask = (rect_max >= tile_min) & (rect_min <= tile_max)
                in_mask = in_mask.all(dim=1)
                #############################################################################
                #                             END OF YOUR CODE                              #
                #############################################################################
