...
def rand_cutout(x, ratio=0.5):
    ...
    cutout_h = torch.randint(int(H * 0.25), int(H * 0.5) + 1, (1,), device=x.device).item()
    cutout_w = torch.randint(int(W * 0.25), int(W * 0.5) + 1, (1,), device=x.device).item()
    top = torch.randint(0, H - cutout_h + 1, (B,), device=x.device)
    left = torch.randint(0, W - cutout_w + 1, (B,), device=x.device)
